...
import datetime
import time
#from init import sensorBattery, sensorTemp, 

valveInfo = "http://app.plantgroup.co/api/valves/"

response = urllib.request.urlopen(valveInfo).read()
resp2 = str(response)[2:len(str(response))-1]
jsonForm = json.loads(resp2)[0]
print(jsonForm)
valve_id = jsonForm["id"]
mcu_id = jsonForm["mcu_id"]
print("Valve ID: " + str(valve_id))
print("MCU ID: " + str(mcu_id))
# watering delay configuration 
...
#print(jsonForm["config"]["user"]["watering_schedule"])
